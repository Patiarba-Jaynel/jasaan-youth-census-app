...
const ConsolidatedActivityPage = () => {
  ...
  const fetchActivityLogs = async () => {
    try {
      ...
      // Filter to show only consolidated data related activities
      const consolidatedLogs = response.items.filter(log => 
        log.action.toLowerCase().includes('consolidated') || 
        log.action.toLowerCase().includes('import')
      );
      setActivityLogs(consolidatedLogs);
    } catch (error) {
        ...
        <div className="container px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-center mb-8">
            <div>
              ...
              >
                ...
                Back to Consolidated Dashboard
              </Button>
              <h1 className="text-3xl font-bold">Consolidated Data Activity & Batch Management</h1>
              <p className="text-muted-foreground">
                Track all consolidated data operations and batch imports
              </p>
            </div>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Recent consolidated data operations and batch imports
              </CardDescription>
            </CardHeader>
            <CardContent>
              {activityLogs.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-muted-foreground">No consolidated data activities recorded yet.</p>
                </div>
              ) : (
